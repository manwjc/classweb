...
router.post('/login', function(req, res, next) {
	...
	handler(req, res, "user", {
		...
		if(data.length === 0) {
			res.end('{"status":"0004","err":"抱歉，系统中并无该用户，如有需要，请向管理员申请"}');
		} else if(data[0].password !== password) {
			res.end('{"status":"0001","err":"密码不正确"}');
		} else if(data.length !== 0 && data[0].password === password) {

			req.session.username = req.body.username; //存session
			req.session.password = password;

			res.end('{"status":"0000","message":"登录成功","success":"true"}');
		}
...
router.post('/logout', function(req, res, next) {
	...
	res.end('{"status":"0000","message":"成功退出登录"}');
})
